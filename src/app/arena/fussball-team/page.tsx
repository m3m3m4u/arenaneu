...
export default function FussballTeamLobbyPage(){
  const { data: session } = useSession();
  const [list,setList] = useState<LobbyListItem[]>([]);
  const [loadingList,setLoadingList] = useState(false);
  const loadList = useCallback(async()=>{
    setLoadingList(true);
    try { const r = await fetch('/api/fussball-team/lobbies'); const j:LobbyApiResponse = await r.json(); if(j.success) setList(j.lobbies||[]); } catch{} finally { setLoadingList(false); }
  },[]);
  ...
  async function join(id:string){
    ...
    try { const r=await fetch(`/api/fussball-team/lobbies/${id}/join`, { method:'POST' }); const j=await r.json(); if(!j.success) setError(j.error||'Join fehlgeschlagen'); else setLobby(j.lobby); }
    catch(e:any){ setError(String(e)); }
  ...
  async function deleteLobby(id:string){
    setError(undefined);
    try {
      const r = await fetch(`/api/fussball-team/lobbies/${id}/delete`, { method:'POST' });
  ...
  async function leave(){
    if(!lobby) return;
    try { await fetch(`/api/fussball-team/lobbies/${lobby.id}/leave`, { method:'POST' }); }
    finally { setLobby(null); setReady(false); }
    loadList();
  }

  useEffect(()=>{
    if(!lobby?.id) return; let alive=true; const tick=async()=>{ try{ const r=await fetch(`/api/fussball-team/lobbies/${lobby.id}/join`); const j=await r.json(); if(!alive) return; if(j.success){ setLobby((prev:any)=> prev? { ...prev, ...j.lobby }: j.lobby); } }catch{} };
    const iv=setInterval(tick, 4000); tick(); return ()=>{ alive=false; clearInterval(iv); };
  },[lobby?.id]);

  useEffect(()=>{
    if(lobby?.status==='active'){
      router.push(`/arena/fussball-team-live/${lobby.id}`);
    }
      ...
      <main className="max-w-6xl mx-auto px-4 md:px-6 py-4 flex flex-col gap-6">
        ...
        <div className="rounded border bg-white shadow p-4 flex flex-col gap-4 max-w-5xl">
          ...
          <div className="flex flex-col gap-2">
            ...
            {lobby.status==='active' && (
              <a href={`/arena/fussball-team-live/${lobby.id}`} className="inline-block mt-1 px-3 py-1 text-xs rounded bg-blue-600 text-white hover:bg-blue-700">Zum Spiel »</a>
            )}
          </div>
