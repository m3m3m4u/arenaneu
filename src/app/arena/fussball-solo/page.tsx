...
export default function FussballSoloProto(){
  ...
  useEffect(()=>{ let frame:number; function loop(ts:number){ if(!running){ frame=requestAnimationFrame(loop); return; } const dt=(ts-lastTime.current)/1000||0; lastTime.current=ts; step(dt); draw(); frame=requestAnimationFrame(loop);} frame=requestAnimationFrame(loop); return()=>cancelAnimationFrame(frame); },[running,size.w,size.h]);
  ...
  function draw(){ const cvs=canvasRef.current; if(!cvs)return; const ctx=cvs.getContext('2d'); if(!ctx)return; const {w,h}=size; ctx.clearRect(0,0,w,h); ctx.fillStyle='#0b7d2b'; ctx.fillRect(0,0,w,h); ctx.strokeStyle='#ffffff'; ctx.lineWidth=2; ctx.beginPath(); ctx.moveTo(w/2,0); ctx.lineTo(w/2,h); ctx.stroke(); ctx.beginPath(); ctx.arc(w/2,h/2,60,0,Math.PI*2); ctx.stroke(); ctx.strokeStyle='#ffd700'; ctx.strokeRect(0,h/2-60,30,120); ctx.strokeRect(w-30,h/2-60,30,120); const b=ball.current; ctx.fillStyle='#ffffff'; ctx.beginPath(); ctx.arc(b.pos.x,b.pos.y,b.r,0,Math.PI*2); ctx.fill(); ctx.strokeStyle='#222'; ctx.stroke(); }
