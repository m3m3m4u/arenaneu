...
export default function FussballLivePage(){
  ...
  // Falls Session später kommt oder Seite wechselt: Side gelegentlich aktualisieren
  useEffect(()=>{
    let alive = true;
    (async()=>{
      try{
        const uid = String((session as any)?.user?.id || (session as any)?.user?._id || '');
        if(!uid) return;
        const rLobby = await fetch(`/api/fussball/lobbies/${encodeURIComponent(id)}/join`, { cache: 'no-store' });
        const jLobby = await rLobby.json();
          ...
          <div className="absolute inset-0 flex flex-col text-white">
            ...
            <div style={{height:'25%'}} className="flex items-center justify-between px-4 md:px-6">
              ...
              <div className="w-16 text-right">
                {/* Hinweis zur eigenen Seite in Vollbild – simpel via localStorage Flag (optional) */}
              </div>
            </div>
