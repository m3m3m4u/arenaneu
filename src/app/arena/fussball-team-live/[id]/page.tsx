...
export default function FussballTeamLivePage(){
  ...
  useEffect(()=>{
    ...
    (async()=>{
      try{
        ...
        const rLobby = await fetch(`/api/fussball-team/lobbies/${encodeURIComponent(id)}/join`);
  ...
  async function answer(idx:number){
    ...
    try{
      const r = await fetch(`/api/fussball-team/lobbies/${encodeURIComponent(id)}/answer`, { method:'POST', headers:{ 'Content-Type':'application/json' }, body: JSON.stringify({ correct: isCorrect }) });
  ...
  useEffect(()=>{
    ...
    async function tick(){
      try{
        const r = await fetch(`/api/fussball-team/lobbies/${encodeURIComponent(id)}/state`, { cache:'no-store' });
        const j = await r.json();
        if(!alive) return;
        if(j?.success && j.state){ setScores(j.state.scores); setGoals(j.state.goals); setFieldIdx(j.state.fieldIdx); setTurn(j.state.turn); }
    ...
    <main className="max-w-6xl mx-auto p-4 md:p-6">
      <header className="mb-4 grid grid-cols-1 md:grid-cols-3 gap-3 items-end">
        <div className="flex flex-col gap-1">
          <h1 className="text-2xl font-bold">⚽ Fußball Team Match</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              <>
                ...
                <div className="flex flex-col gap-3">
                  {current.options.map((opt,i)=>{
                    ...
                    } else if(picked){ cls=base+' border-blue-500 bg-blue-50'; }
        ...
        <FieldView images={FIELD_IMAGES} fieldIdx={fieldIdx} fieldWH={fieldWH} goals={goals} scores={scores} current={current} locked={locked} answerState={answerState} onAnswer={answer} />
      </div>
    </main>
  );
}

function FieldView({ images, fieldIdx, fieldWH, goals, scores, current, locked, answerState, onAnswer }:{ images:string[]; fieldIdx:number; fieldWH:{w:number;h:number}|null; goals:{left:number;right:number}; scores:{left:number;right:number}; current:MCQuestion|undefined; locked:boolean; answerState:{picked:number|null; correct:boolean|null}; onAnswer:(idx:number)=>void }){
  ...
  const scale = 0.6;
  const enterFs = async()=>{ try{ const el = containerRef.current; if(!el) return; if(el.requestFullscreen){ await el.requestFullscreen(); setIsFs(true); } }catch{} };
  const exitFs = async()=>{ try{ if(document.fullscreenElement){ await document.exitFullscreen(); } setIsFs(false); }catch{} };
  useEffect(()=>{ const h=()=> setIsFs(Boolean(document.fullscreenElement)); document.addEventListener('fullscreenchange', h); return ()=> document.removeEventListener('fullscreenchange', h); },[]);
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div ref={containerRef} className={"relative border rounded bg-black shadow-inner overflow-hidden mx-auto " + (isFs? 'fixed inset-0 z-50 rounded-none border-0 mx-0':'')} style={isFs? { width: '100%', height: '100%' } : { aspectRatio: aspect, width: `${Math.round(100*scale)}%` }}>
        {!isFs && (<>
                  ...
                  {current ? (<>
                    ...
                    <div className="flex flex-col gap-3">
                      {current.options.map((opt,i)=>{
                        ...
                        } else if(picked){ cls=base+' border-blue-400 bg-blue-500/20 text-blue-100'; }
