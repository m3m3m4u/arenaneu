...
export async function joinLobby(id: string, userId: string, username: string){
  ...
  // Startbedingung: mindestens 2 Spieler gesamt und je Team mindestens 1 bereit
  const hasLeft = lobby.players.some(p=>p.side==='left' && p.ready);
  const hasRight = lobby.players.some(p=>p.side==='right' && p.ready);
...
export async function updateReady(id: string, userId: string, ready: boolean){
  ...
  const hasLeft = lobby.players.some(pl=>pl.side==='left' && pl.ready);
  const hasRight = lobby.players.some(pl=>pl.side==='right' && pl.ready);
  if(lobby.status==='waiting' && hasLeft && hasRight) lobby.status='active';
...
export async function applyAnswer(id: string, isCorrect: boolean, answeredBy: 'left'|'right'){
  ...
  if(trailingSide){
    ...
    if(deficit >= 4) stepThreshold = 1; else if(deficit >= 2) stepThreshold = 2;
  }
